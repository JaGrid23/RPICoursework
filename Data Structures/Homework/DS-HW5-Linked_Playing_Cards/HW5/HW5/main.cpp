...
void StudentTests() {
    ...
    Node* test1 = CreateDeck(2);
    Node* test2 = CreateDeck(21);
    
    //Printing the starting decks:
    std::cout<<std::endl<<"Printing starting decks: "<<std::endl;
    PrintDeckPrimary("Deck 1: (size: 2) ", test1);
    PrintDeckPrimary("Deck 2: (size: 21) ", test2);
    
    //DECK SIZE TESTS
    std::cout<<std::endl<<"Testing Deck Size Function: "<<std::endl;
    std::cout<<"Deck 1 (2): "<<DeckSize(test1)<<std::endl;
    std::cout<<"Deck 2 (21): "<<DeckSize(test2)<<std::endl;
    
    //COPY DECK TESTS
    std::cout<<std::endl<<"Testing Copy Function: "<<std::endl;
    Node* copy2 = CopyDeck(test2);
    PrintDeckPrimary("Copy Deck 2: (size: 21) ", copy2);
    
    //CUT DECK AND SHUFFLE TEST
    std::cout<<std::endl<<"Testing Cut Deck Function:"<<std::endl;
    Node *top, *bottom;
    CutDeck(test2,top,bottom,"perfect");
    std::cout<<std::endl<<"Testing Shuffle Function:"<<std::endl;
    test2 = Shuffle(top,bottom,"perfect");
    PrintDeckPrimary("Deck 2:",test2);
    
    //DEAL FUNCTION TEST
    std::cout<<std::endl<<"Testing Deal Function:"<<std::endl;
    Node* hands[3];
    Deal(test2,hands,3,"one-at-a-time",15);
    // print the decks
    PrintDeckPrimary("Hand 1:",hands[0]);
    PrintDeckPrimary("Hand 2:",hands[1]);
    PrintDeckPrimary("Hand 3:",hands[2]);
    
    //SORT HAND FUNCTION TEST
    std::cout<<std::endl<<"Testing Sort Hand Function:"<<std::endl;
    Node* sorted_hands[3];
    sorted_hands[0] = SortHand(hands[0]);
    sorted_hands[1] = SortHand(hands[1]);
    sorted_hands[2] = SortHand(hands[2]);
    
    PrintDeckSorted("Hand 1 (sorted):",sorted_hands[0]);
    PrintDeckSorted("Hand 2 (sorted):",sorted_hands[1]);
    PrintDeckSorted("Hand 3 (sorted):",sorted_hands[2]);
    
    //Cleaning up my memory
    DeleteAllCards(hands[0]);
    DeleteAllCards(hands[1]);
    DeleteAllCards(hands[2]);
    DeleteAllCards(test2);
    
    DeleteAllCards(test1);
    DeleteAllCards(copy2);
    
    
    //    //SHUFFLE DECK TESTS
    //    std::cout<<std::endl<<"Testing Shuffle Function:"<<std::endl;
    //    Node* shuffled_1 = Shuffle(first_part1, second_part1, "perfect");
    //    PrintDeckPrimary("Test 1 (size: 13): ", shuffled_1);
    //
    //    Node* shuffled_2 = Shuffle(first_part2, second_part2, "perfect");
    //    PrintDeckPrimary("Test 2: ", shuffled_2);
    //
    //    //SAME PRIMARY ORDER TEST
    //    std::cout<<std::endl<<"Testing Same Primary Order:"<<std::endl;
    //    if(SamePrimaryOrder(test1, test4)){
    //        cout<<"Deck 1 and Deck 4: MATCH"<<std::endl;
    //    }
    //    else{
    //        cout<<"Deck 1 and Deck 4: DO NOT MATCH"<<std::endl;
    //    }
    //
    //    //DEAL TESTS
    //    std::cout<<std::endl<<"Testing Deal Function:"<<std::endl;
    //    Node* hands[3];
    //    Deal(test2, hands, 3, "one-at-a-time", 7);
    //    PrintDeckPrimary("Hand 1: ", hands[0]);
    //    PrintDeckPrimary("Hand 2: ", hands[1]);
    //    PrintDeckPrimary("Hand 3: ", hands[2]);
    //
    ...
    //    Node* sorted_hands[3];
    //    for(int i=0; i<3; i++){
    //        sorted_hands[i] = SortHand(hands[i]);
    //    }
    //    PrintDeckPrimary("Sorted Hand 1: ", hands[0]);
    //    PrintDeckPrimary("Sorted Hand 2: ", hands[1]);
    //    PrintDeckPrimary("Sorted Hand 3: ", hands[2]);
    //
    //    //Delete everything to clean up memory
    //    DeleteAllCards(test2);
    //    DeleteAllCards(test4);
    //    DeleteAllCards(test5);
    //    for(int i =0; i<3; i++){
    //        DeleteAllCards(hands[i]);
    //    }
    //    DeleteAllCards(first_part1);
    //    DeleteAllCards(first_part2);
    //    DeleteAllCards(first_part3);
    //    DeleteAllCards(second_part1);
    //    DeleteAllCards(second_part2);
    //    DeleteAllCards(second_part3);
}
