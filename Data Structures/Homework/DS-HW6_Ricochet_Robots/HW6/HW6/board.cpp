...
bool Board::getVerticalWall(int r, double c) const {
  ...
  // subtract one and round down because the corner is (0,0) not (1,1)
  return vertical_walls[r-1][trunc(c)];
}
...
bool Board::moveRobot(int i, const std::string &direction) {
    //Determining the increment direction:
    int r_incr = robots[i].pos.row;
    int c_incr = robots[i].pos.col;
    if(direction == "north"){ r_incr -=1; }
    else if(direction == "east"){ c_incr += 1; }
    else if(direction == "south"){ r_incr += 1; }
    else if(direction == "west"){ c_incr -= 1; }
    
    //Check if it hit an outer wall
    if(r_incr<=0 || c_incr<=0 || r_incr>rows || c_incr>cols){
        return false;
    }
    
    //Check if it hit another robot
    for(int j = 0; j<robots.size(); j++){
        if(j!=i && (robots[j].pos.row==r_incr && robots[j].pos.col==c_incr)){
            return false;
        }
    }
    //Check if it hit a wall
    if(direction == "north" && getHorizontalWall(r_incr+0.5, c_incr)){ return false; }
    else if(direction == "south" && getHorizontalWall(r_incr-0.5, c_incr)){ return false; }
    else if(direction == "west" && getVerticalWall(r_incr, c_incr+0.5)){ return false; }
    else if(direction == "east" && getVerticalWall(r_incr, c_incr-0.5)){return false; }
    
    //If does not hit stopping point, keep moving
    setspot(robots[i].pos, ' ');                //Remove old spot
    robots[i].pos.row = r_incr;
    robots[i].pos.col = c_incr;
    setspot(robots[i].pos, robots[i].which);    //Set new spot
