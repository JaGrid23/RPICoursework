...
int list_operations(int &secret_code) {
 ...
 for(std::list<char>::reverse_iterator itr = l1.rbegin(); itr != l1.rend(); itr++) {
...
int pythagoras(int x, int y) {
  ...
  float fracpart = modf(sqrt(sumsquares), placeholder); int p = *placeholder;
    if(fracpart == 0){ delete placeholder;
        return p;}

  // x is the hypotenuse, need to subtract instead of add
  float diffsquares = abs(y*y - x*x);
  fracpart = modf(sqrt(diffsquares), placeholder);  int q = *placeholder;
    if(fracpart == 0){ delete placeholder;
        return q;}

  // no triple exists
    else{delete placeholder; return -1;}
}
...
const std::string& file_operations(int argc, char** argv, char*& returned_buffer, int& retlen) {
    ...
    static std::string answer = "SUCCESS"; static std::string bad_arg = "ERROR: bad arguments";
...
float multidivide(int numerator, int d1, int d2, int d3, int d4) {
  float f2 = ((float((numerator / d1) / d2) / d3) / d4);
  return f2;
}
...
int main(int argc, char* argv[]) {
  ...
  const std::string ops(argv[1]);       //Should be argv[1]
  const char* outFileName = argv[3];    //Should be argv[3]
